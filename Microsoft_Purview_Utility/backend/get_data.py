...
class PurviewSearchClient:
    ...
    def get_collections_with_names(self):
        """Get all collections with their IDs and friendly names from Azure Purview.
        
        Returns:
            dict: Dictionary mapping collection ID (name) to friendly name
        """
        url = f"{self.config.purview_endpoint}/collections?api-version=2019-11-01-preview"
        headers = {
            "Authorization": f"Bearer {self.get_access_token()}",
        ...
        while next_link:
            ...
            for collection in collections:
                ...
                friendly_name = collection.get("friendlyName", collection_id)
                if collection_id:
                    collection_mapping[collection_id] = friendly_name
        ...
        for collection_id in collection_ids:
            ...
            try:
                ...
                while True:
                    ...
                    if response["value"]:
                        # Convert SearchResultValue objects to proper dictionaries
                        ...
                        for item in value_list:
                            if hasattr(item, 'as_dict'):
                                ...
                                value_dicts.append(item.as_dict())
                            elif isinstance(item, dict):
                                value_dicts.append(item)
                            else:
                                # Fallback: convert to dict
                                value_dicts.append(dict(item))
                        
                        # Log all retrieved IDs
                        ids_in_request = [entity.get('id') if isinstance(entity, dict) else getattr(entity, 'id', None) for entity in value_list]

                        current_page_count = len(value_dicts)

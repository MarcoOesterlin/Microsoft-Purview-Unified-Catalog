...
def main():
    """Main execution function for the data extraction and deletion process.
    ...
    5. Deletes assets from the specified collection
    """
    ...
    if search_results is not None:
        ...
        try:
            print(jdf)
            # Filter for collectionId
            filtered_jdf = jdf[jdf['collectionId'] == 'ADD COLLECTION ID HERE']
            # Show only the GUIDs (id column)
            deletion = filtered_jdf['id']
            print(deletion)
            print(f"Number of GUIDs in deletion: {deletion.count()}")
        
            # Loop through each GUID and delete the asset, renewing token every 5000 assets
            token = purview_client.get_access_token()
            headers = {
                "Authorization": f"Bearer {token}",
                "Content-Type": "application/json"
            }
            endpoint = purview_config.purview_endpoint 

            for idx, guid in enumerate(deletion):
                # Renew token every 5000 assets
                ...
                if response.status_code == 200:
                    print(f"Successfully deleted asset with GUID: {guid}")
                else:
                    print(f"Failed to delete asset with GUID: {guid}. Status code: {response.status_code}, Response: {response.text}")
